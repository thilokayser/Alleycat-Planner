...
async function runAlleycatTestSuite(){
  ...
  /* 4c) Paket 6 (Phase 21): Spezielle Rennformate — Cargo-Modul,
     Trackbike-Attribute, Rätsel-Sheet-PDF-Block (race-formats.js). Beide
     Sub-Features sind reine Feature-Registry-Sichtbarkeits-Flags (Default
     aus), daher zuerst aktivieren. */
  {
    check('FEATURE_REGISTRY enthält "cargo_module"', !!featureRegistryEntry('cargo_module'));
    check('FEATURE_REGISTRY enthält "trackbike_attributes"', !!featureRegistryEntry('trackbike_attributes'));
    checkEqual('cargo_module ist standardmäßig deaktiviert', isFeatureEnabled('cargo_module', evt), false);
    checkEqual('trackbike_attributes ist standardmäßig deaktiviert', isFeatureEnabled('trackbike_attributes', evt), false);
    toggleFeature('cargo_module');
    checkEqual('toggleFeature aktiviert cargo_module', isFeatureEnabled('cargo_module', evt), true);
    toggleFeature('trackbike_attributes');
    checkEqual('toggleFeature aktiviert trackbike_attributes', isFeatureEnabled('trackbike_attributes', evt), true);

    /* Checkpoint: cargoItem-Feld + CRUD */
    checkEqual('withCheckpointDefaults setzt cargoItem auf null', withCheckpointDefaults({}).cargoItem, null);
    const cargoCp = evt.checkpoints[0];
    setCpCargoItem(cargoCp.id, true);
    check('setCpCargoItem legt withCargoItemDefaults an', !!cargoCp.cargoItem && cargoCp.cargoItem.name === '');
    onCpCargoItemFieldChange(cargoCp.id, 'name', 'Sack Mehl');
    onCpCargoItemFieldChange(cargoCp.id, 'weightKg', '5');
    onCpCargoItemFieldChange(cargoCp.id, 'volumeUnits', '2');
    onCpCargoItemFieldChange(cargoCp.id, 'bonusPoints', '10');
    checkEqual('onCpCargoItemFieldChange setzt Namen', cargoCp.cargoItem.name, 'Sack Mehl');
    checkEqual('onCpCargoItemFieldChange setzt Gewicht (numerisch)', cargoCp.cargoItem.weightKg, 5);
    checkEqual('onCpCargoItemFieldChange setzt Volumen', cargoCp.cargoItem.volumeUnits, 2);
    checkEqual('onCpCargoItemFieldChange setzt Bonuspunkte', cargoCp.cargoItem.bonusPoints, 10);
    onCpCargoItemFieldChange(cargoCp.id, 'weightKg', '-3');
    checkEqual('onCpCargoItemFieldChange klemmt negative Werte auf 0', cargoCp.cargoItem.weightKg, 0);
    onCpCargoItemFieldChange(cargoCp.id, 'weightKg', '5');
    setCpCargoItem(cargoCp.id, false);
    checkEqual('setCpCargoItem(false) entfernt cargoItem wieder', cargoCp.cargoItem, null);
    setCpCargoItem(cargoCp.id, true);
    onCpCargoItemFieldChange(cargoCp.id, 'name', 'Sack Mehl');
    onCpCargoItemFieldChange(cargoCp.id, 'weightKg', '5');
    onCpCargoItemFieldChange(cargoCp.id, 'bonusPoints', '10');

    /* Rider-Felder: Defaults + Handler */
    checkEqual('withRiderDefaults setzt cargoCapacityKg auf 0', withRiderDefaults({}).cargoCapacityKg, 0);
    checkEqual('withRiderDefaults setzt gearRatio auf leer', withRiderDefaults({}).gearRatio, '');
    checkEqual('withRiderDefaults setzt isBrakeless auf false', withRiderDefaults({}).isBrakeless, false);
    checkEqual('withRiderDefaults setzt isWorkbike auf false', withRiderDefaults({}).isWorkbike, false);

    const cargoRider = evt.riders[0];
    onRiderCargoCapacityChange(cargoRider.bib, '10');
    checkEqual('onRiderCargoCapacityChange setzt Kapazität', cargoRider.cargoCapacityKg, 10);
    onRiderGearRatioChange(cargoRider.bib, '48x17');
    checkEqual('onRiderGearRatioChange setzt Übersetzung', cargoRider.gearRatio, '48x17');
    onRiderBrakelessChange(cargoRider.bib, true);
    checkEqual('onRiderBrakelessChange setzt Flag', cargoRider.isBrakeless, true);
    onRiderWorkbikeChange(cargoRider.bib, true);
    checkEqual('onRiderWorkbikeChange setzt Flag', cargoRider.isWorkbike, true);

    /* Cargo-Lieferung: Summary, Toggle, Kapazitätswarnung, Bonuspunkte */
    checkEqual('riderCargoCheckpoints: kein Frachtgut ohne abgeschlossenen Checkpoint', riderCargoCheckpoints(evt, cargoRider).length, 0);
    cargoRider.completed = (cargoRider.completed || []).concat(cargoCp.id);
    checkEqual('riderCargoCheckpoints erkennt Frachtgut an abgeschlossenem Checkpoint', riderCargoCheckpoints(evt, cargoRider).length, 1);

    let cargoSummaryCheck = computeCargoDeliverySummary(evt, cargoRider);
    check('computeCargoDeliverySummary: 1 Item, standardmäßig geliefert', cargoSummaryCheck.items.length === 1 && cargoSummaryCheck.items[0].delivered);
    checkEqual('computeCargoDeliverySummary: Gesamtgewicht', cargoSummaryCheck.totalWeightKg, 5);
    checkEqual('computeCargoDeliverySummary: Bonuspunkte-Summe', cargoSummaryCheck.totalBonusPoints, 10);
    check('computeCargoDeliverySummary: 10kg Kapazität, 5kg geliefert -> nicht überladen', !cargoSummaryCheck.overCapacity);

    onCargoDeliveryToggle(cargoRider.bib, cargoCp.id, false);
    cargoSummaryCheck = computeCargoDeliverySummary(evt, cargoRider);
    checkEqual('onCargoDeliveryToggle: als nicht geliefert markiert -> 0 Bonuspunkte', cargoSummaryCheck.totalBonusPoints, 0);
    onCargoDeliveryToggle(cargoRider.bib, cargoCp.id, true);

    cargoRider.cargoCapacityKg = 2;
    check('computeCargoDeliverySummary erkennt Kapazitätsüberschreitung', computeCargoDeliverySummary(evt, cargoRider).overCapacity);
    cargoRider.cargoCapacityKg = 10;

    applyCargoBonusPoints(evt, cargoRider);
    checkEqual('applyCargoBonusPoints bucht Bonuspunkte in den Ledger', pointsForRider(evt, cargoRider.bib), 10);
    applyCargoBonusPoints(evt, cargoRider);
    checkEqual('applyCargoBonusPoints räumt vorherige Cargo-Buchungen auf (kein Verdoppeln)', pointsForRider(evt, cargoRider.bib), 10);
    check('Ledger-Eintrag trägt source "cargo"', ledgerEntriesForRider(evt, cargoRider.bib).some(p => p.source === 'cargo'));

    /* Ziel-Check-in-Integration */
    removeLedgerEntries(evt, p => p.riderBib === cargoRider.bib);
    activateCheckinRider(cargoRider.bib);
    confirmRiderAtFinish();
    checkEqual('confirmRiderAtFinish vergibt Cargo-Bonuspunkte bei aktivem Cargo-Modul', pointsForRider(evt, cargoRider.bib), 10);
    unconfirmRiderAtFinish();
    checkEqual('unconfirmRiderAtFinish entfernt Cargo-Bonuspunkte wieder', pointsForRider(evt, cargoRider.bib), 0);
    await wait(20);
    check('Ziel-Check-in zeigt Fracht-Sektion bei aktivem Cargo-Modul', document.querySelector('.checkin-cargo-section') !== null);
    state.checkinActiveBib = null;
    renderCheckin();

    /* Trackbike: Rekorde + Badge + Leaderboard-Filter */
    const otherRider = evt.riders[1];
    otherRider.isBrakeless = false;
    otherRider.isWorkbike = false;
    otherRider.finishTime = toLocalDateTimeInputValue(new Date(Date.now() + 60000));
    otherRider.raceStatus = '';
    cargoRider.finishTime = toLocalDateTimeInputValue(new Date());
    cargoRider.raceStatus = '';
    const brakelessRider2 = evt.riders[2];
    brakelessRider2.isBrakeless = true;
    brakelessRider2.finishTime = toLocalDateTimeInputValue(new Date(Date.now() - 60000));
    brakelessRider2.raceStatus = '';

    const records = computeTrackbikeRecords(evt);
    checkEqual('computeTrackbikeRecords: schnellster Brakeless-Fahrer korrekt ermittelt', records.fastestBrakeless && records.fastestBrakeless.bib, brakelessRider2.bib);
    checkEqual('computeTrackbikeRecords: bestes Lastenrad korrekt ermittelt', records.bestWorkbike && records.bestWorkbike.bib, cargoRider.bib);

    check('raceFormatsBadgeHtml zeigt Bremsenlos-Rekord-Badge', raceFormatsBadgeHtml(evt, brakelessRider2).includes(t('raceFormats.fastestBrakelessBadge')));
    checkEqual('raceFormatsBadgeHtml zeigt kein Badge für Fahrer ohne Rekord', raceFormatsBadgeHtml(evt, otherRider), '');

    checkEqual('riderMatchesStatusFilter "brakeless" filtert korrekt', riderMatchesStatusFilter(brakelessRider2, 'brakeless'), true);
    checkEqual('riderMatchesStatusFilter "brakeless" schließt Nicht-Brakeless aus', riderMatchesStatusFilter(otherRider, 'brakeless'), false);
    checkEqual('riderMatchesStatusFilter "workbike" filtert korrekt', riderMatchesStatusFilter(cargoRider, 'workbike'), true);

    openLeaderboard();
    await wait(20);
    check('Leaderboard-Statusfilter zeigt Bremsenlos-Option bei aktivem Trackbike-Feature', document.querySelector('.leaderboard-status-filter').innerHTML.includes(t('raceFormats.brakelessLabel')));
    clearLeaderboardFilters();
    await wait(20);
    onLeaderboardStatusFilterChange('brakeless');
    await wait(20);
    checkEqual('Leaderboard mit "Bremsenlos"-Filter zeigt nur Brakeless-Fahrer', document.querySelectorAll('#view-leaderboard .lb-row').length, evt.riders.filter(r => r.isBrakeless).length);
    clearLeaderboardFilters();
    await wait(20);

    /* Rider-Card / Checkpoint-Edit-Form UI (nur sichtbar bei aktiviertem Feature) */
    openRiders();
    await wait(20);
    check('Fahrerliste zeigt Cargo-Kapazitätsfeld bei aktivem Cargo-Modul', document.querySelector('.rider-cargo-row') !== null);
    check('Fahrerliste zeigt Trackbike-Felder bei aktivem Trackbike-Feature', document.querySelector('.rider-trackbike-row') !== null);
    toggleFeature('cargo_module');
    toggleFeature('trackbike_attributes');
    render();
    await wait(20);
    check('Fahrerliste blendet Cargo-Feld nach Deaktivierung aus', document.querySelector('.rider-cargo-row') === null);
    check('Fahrerliste blendet Trackbike-Felder nach Deaktivierung aus', document.querySelector('.rider-trackbike-row') === null);
    check('Deaktivierung löscht die Fahrer-Daten nicht (reine Sichtbarkeits-Flag)', cargoRider.cargoCapacityKg === 10 && cargoRider.gearRatio === '48x17');
    toggleFeature('cargo_module');
    toggleFeature('trackbike_attributes');

    /* Rätsel-Sheet PDF-Block (21.3) */
